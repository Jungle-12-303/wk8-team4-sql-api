...
| 입력 | `.sql` 파일, `--interactive`, `--benchmark`, `--server` |
| 출력 | 표준 출력 + `.csv` 파일 |
| 지원 문장 | `INSERT`, `SELECT` |
| 지원 WHERE | 단일 조건 `=`, `>`, `<`, `!=` |
...
벤치마크가 끝나면 프로그램은 종료됩니다. 벤치마크 데이터는
`data_dir/benchmark/` 아래에 따로 남습니다.
...
- API 서버 모드: `./build/sqlproc --schema-dir ./examples/schemas --data-dir /tmp/sqlproc-data --server --port 8080`
- interactive 모드에서는 `sqlproc>` 프롬프트에서 SQL 한 줄씩 입력하고 `.exit`로 종료합니다.
- benchmark 모드에서는 더미 데이터 개수를 먼저 입력하고, 결과 표를 출력한 뒤 종료합니다.
- API 서버 모드는 `GET /health`, `POST /query`만 지원합니다. thread pool로 연결 요청을 처리하지만,
  기존 SQL 엔진 공유 상태를 보호하기 위해 SQL 실행 구간은 mutex로 한 번에 하나씩 실행합니다.

### HTTP API 예시

```bash
mkdir -p /tmp/sqlproc-data
./build/sqlproc \
  --schema-dir ./examples/schemas \
  --data-dir /tmp/sqlproc-data \
  --server \
  --port 8080
```
...
새 서버 기능이 어떤 CS:APP PDF 절을 바탕으로 구현됐는지는
`docs/wk08_api_server_pdf_concepts.md`에 정리했습니다.
