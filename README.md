...
|---|---|---|---|
| 개념 | 한 번에 하나만 진입시키는 상호 배제 잠금 | 카운터로 N개 스레드 동시 진입을 허용하는 신호 기법 | 읽기는 다수 허용, 쓰기는 하나만 허용하는 분리된 잠금 |
| 동시 접근 | 불가 (항상 1개) | 가능 (최대 N개) | 읽기 다수 동시 가능, 쓰기는 1개 |
| 주요 용도 | 단순 공유 자원 보호 | 자원 풀, 생산자-소비자 패턴 | 읽기 빈번·쓰기 드문 공유 자원 |
| 단점 | 읽기도 직렬화 → 불필요한 대기 | 획득/해제 순서 보장 없어 오용 쉬움 | 구현에 따라 writer starvation 가능 |

**우리가 RwLock을 선택한 이유**

SELECT 요청은 테이블을 수정하지 않으므로 여러 SELECT가 동시에 실행되어도 data race가 없습니다. Mutex를 쓰면 SELECT끼리도 직렬화되어 불필요한 대기가 생깁니다.

RwLock은 SELECT에 shared read lock, INSERT에 exclusive write lock을 분리 적용합니다. SELECT 트래픽이 많을수록 Mutex 대비 병렬 처리량이 올라가고, INSERT는 write lock으로 테이블 변경을 직렬화하여 data race를 막습니다.
