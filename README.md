...
- 기존 SQL 엔진 재사용: `sql.c`, `table.c`, `bptree.c`는 lock-unaware 상태를 유지합니다.
...
- `server.c`: CLI 하네스 + HTTP 서버 엔트리포인트
- `http_server.c/h`: socket accept, queue, worker, shutdown 처리
- `api.c/h`: HTTP 파싱, JSON 응답 직렬화, 오류 매핑
- `db_server.c/h`: shared table, rwlock, metrics, query 분류
- `platform.c/h`: Windows/POSIX 공통 스레드/락 래퍼
- `sql.c/h`: SQL 파싱과 실행
- `table.c/h`: 테이블 저장, 선형 탐색, 인덱스 연동
- `bptree.c/h`: PK 인덱스용 B+Tree
...
3. `sql.c`, `table.c`, `bptree.c`는 lock-unaware로 두고 서버 경계에서만 동기화합니다.
...
```bash
gcc -std=c11 -Wall -Wextra -Werror -pedantic -O2 -o unit_test.exe unit_test.c db_server.c api.c platform.c bptree.c table.c sql.c
...
```bash
./server --query "INSERT INTO users VALUES ('Alice', 20);" \
...
```bash
powershell -ExecutionPolicy Bypass -File .\server_cli_smoke_test.ps1
```
...
자세한 절차는 [docs/http-smoke-test.md](C:/Users/home/workspace/jungle/wedcoding/week8/realdbms/wk8-team4-sql-api/docs/http-smoke-test.md)에 정리했습니다.

## 현재 확인 상태

- `unit_test.exe`는 현재 작업 세션에서 통과 확인
- `server_cli_smoke_test.ps1`, `server_http_smoke_test.ps1`는 저장소에 포함
- 다만 이 Codex 데스크톱 환경에서는 Device Guard / App Control 정책 때문에 `server.exe` 실행 자체가 차단되어 smoke 스크립트를 여기서 끝까지 실행하지는 못했습니다
