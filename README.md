...
- Docker Compose 기반 unit test/smoke 경로와 benchmark 타깃 포함
...
tests/
  ...
  smoke/     HTTP smoke 가이드, Postman collection, 레거시 스크립트
...
- `tests/smoke/`: HTTP smoke 가이드, Postman collection, 레거시 스크립트
- `benchmarks/`: 성능 확인용 벤치마크
...
이 저장소는 Docker 기반 Linux 환경만 지원합니다. 호스트 OS가 Windows, macOS, Linux 중 무엇이든 상관없이 빌드와 실행은 Docker 또는 VS Code Dev Container 안에서 진행하는 것을 기준으로 합니다.

### 준비물

- Docker Engine 또는 Docker Desktop
- Docker Compose v2
- 선택 사항: VS Code + Dev Containers 확장
...
- `.devcontainer/devcontainer.json`: VS Code `Reopen in Container`용 개발 컨테이너 설정

### Docker Compose로 빌드

```bash
docker compose build sql-api
docker compose --profile test build unit-test
```
...
아래 `./build/bin/*` 명령은 VS Code Dev Container 안에서 직접 실행하거나, Docker build stage 안에서 만들어지는 바이너리를 기준으로 설명합니다.
...
VS Code에서 왼쪽 아래 Dev Container 표시와 `Reopen in Container`를 쓰려면 Dev Containers 확장이 설치되어 있어야 합니다.

1. VS Code에 `Dev Containers` 확장을 설치합니다.
2. 이 저장소를 연 뒤 Command Palette에서 `Dev Containers: Reopen in Container`를 실행합니다.
3. `.devcontainer/devcontainer.json`이 `Dockerfile`의 `devcontainer` target으로 개발 컨테이너를 빌드합니다.
4. 컨테이너가 열리면 `postCreateCommand`로 `make unit_test server`가 자동 실행됩니다.

컨테이너 안에서는 기존과 같은 명령을 그대로 사용할 수 있습니다.
...
Dev Container 안에서 직접 확인하려면 아래 명령을 사용할 수 있습니다.
...
### Docker Compose smoke 확인

`sql-api` 서비스를 띄운 뒤 `curl` 또는 Postman collection으로 기본 API 계약을 end-to-end로 확인할 수 있습니다.
...
이 smoke 확인의 목적은 라우팅, JSON body 파싱, SQL 실행, 오류 매핑, 운영 메트릭 집계가 end-to-end로 맞는지 빠르게 보는 것입니다.
...
`queue_full`처럼 동시 요청이 필요한 시나리오는 Postman 컬렉션보다 별도 burst 요청이나 수동 부하 확인이 더 적합합니다.
...
docker build -t mini-dbms-sql-api .
docker run --rm -p 8080:8080 mini-dbms-sql-api \
  --serve --port 8080 --workers 4 --queue 64 \
...
완전히 깨진 raw HTTP 헤더/바디 framing은 수동 socket 테스트가 더 적합합니다.

## 현재 확인 상태

- `docker compose --profile test run --rm unit-test`는 현재 작업 세션에서 통과 확인
- `docker compose up --build -d sql-api` 뒤 `health`, `insert`, `select`, `metrics` 요청을 현재 작업 세션에서 통과 확인
- `build/bin/unit_test`는 Dev Container/로컬 Linux 세션에서 빠른 회귀 확인용입니다
- `build/bin/server --query ...` CLI 하네스는 Dev Container 안에서 기본 흐름 확인용입니다
- `tests/smoke/server_http_smoke_test.postman_collection.json`, `tests/smoke/server_http_edge_burst_test.postman_collection.json`는 저장소에 포함
- `docker compose up --build sql-api` 뒤 `curl` 또는 Postman collection으로 기본 API 계약과 edge/burst 시나리오를 확인할 수 있습니다
